...
(function(){
  ...
  // === Inject CSS pentru loader brand-native ===
  ...
  // === Hide existing builtin preloader ===
  const existing=document.getElementById('pre');
  if(existing){ existing.style.display='none'; }

  // === Progress simulation (real loading state tied to window.load) ===
  let progress=0;
  let target=0;
  ...
  // Fake progressive load until window.load fires
  let fakeStep = 0;
