...
(function(){
  ...
  // === Style overlay pentru fallback transition ===
  ...
  style.textContent=`
    ...
    /* View Transitions native — Chrome 111+ */
    ...
    /* Shared element view-transition-name pentru portfolio→case morph */
    .pf-img[data-pt-name] { view-transition-name: var(--pt-name); }
  `;
  document.head.appendChild(style);

  // === Inject overlay pentru fallback ===
  const overlay=document.createElement('div');
  overlay.className='ptv2-overlay';
  document.body.appendChild(overlay);

  // === Smart prefetch on hover (>120ms intent signal) ===
  ...
  // === Click interceptor — orchestrate transition ===
  const supportsViewTrans = typeof document.startViewTransition === 'function';

  document.addEventListener('click', e=>{
    if(e.defaultPrevented)return;
    if(e.button!==0||e.metaKey||e.ctrlKey||e.shiftKey||e.altKey)return; // respect modifier keys (open new tab etc)
    const link=e.target.closest && e.target.closest('a[href]');
    if(!link)return;
    ...
    if(url.origin!==location.origin)return;
    if(url.pathname===location.pathname && url.search===location.search) return; // same page hash already handled

    e.preventDefault();

    // Tag shared element if relevant (portfolio image → case study hero)
    const sharedImg=link.querySelector('img.pf-img-img, .pf-img img');
    if(sharedImg){
      sharedImg.style.viewTransitionName='pf-shared-hero';
    }

    // === View Transitions API (native, Chrome 111+) ===
    ...
    // === Fallback: custom fade+scale orchestrated ===
  ...
  // === On enter (page load) — fade in cu reverse animation ===
  // Doar daca user navigates from another page (referrer same origin)
