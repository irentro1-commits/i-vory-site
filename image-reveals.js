...
(function(){
  ...
  function init(){
    ...
    // === IR-1: Image clip-path reveal pe portofoliu cards ===
    ...
    // === IR-2: H2 .sh line-by-line text reveal cu mask ===
    // Pentru fiecare h2 — wrap each line cu div overflow:hidden + animate translateY 100% → 0
    const h2s=document.querySelectorAll('.sh, .contact-h, .reviews-cta-title');
    h2s.forEach(h=>{
      if(h.dataset.irDone)return;
      h.dataset.irDone='1';
      // Process inner — replace text + br nodes cu wrapper lines
    ...
    // === IR-3: Section sub-copy fade-up cu blur (.ss + .s2-desc) ===
    ...
    // === IR-4: Pricing rows mask sweep (price-table tr) ===
    ...
    // === IR-5: How-item & dce-item staggered slide-in ===
