...
(function(){
  ...
  // === K1: HERO H1 SPLITTEXT AMPLIFY ===
  // Existing gsap-kinetic.js handles split. Aici amplific cu staggered char reveal pe ENTRY (top of page).
  function initHeroAmplify(){
    if(!window.gsap)return;
    const heroH=document.querySelectorAll('.s2 h2, .s2 .s2-tag');
    heroH.forEach(el=>{
      // Skip daca already split
      if(el.dataset.splitDone)return;
      el.dataset.splitDone='1';
      const html=el.innerHTML;
      // Split numai text nodes (preserve <br>, <span>, etc)
  ...
  // === K2: SECTION HEADERS VERTICAL ACCENT LINE ===
  ...
  function initIdleAnims(){
    ...
    resetIdle();
    // Inject CSS pentru idle state — earth pulse subtle, dragon glow breathes
  ...
  // === K5: SCROLL PROGRESS INDICATOR top bar ===
